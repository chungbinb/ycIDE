...
EXTERN_C INT WINAPI dp1_ProcessNotifyLib_dp1(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define DP1_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    DP1_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_dp1_global_var_fun[] = { DP1_DEF(DP1_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_dp1_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "4BB4003860154917BC7D8230BF4FA58A", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    /*m_nMajorVersion*/         2, // 本库的主版本号, 必须大于0
    /*m_nMinorVersion*/         0, // 本库的次版本号
    /*m_nBuildNumber*/          50, // 构建版本号, 无需对此版本号作任何处理

    /*m_nRqSysMajorVer*/        3, // 所需要的易语言系统的主版本号
    /*m_nRqSysMinorVer*/        6, // 所需要的易语言系统的次版本号
    /*m_nRqSysKrnlLibMajorVer*/ 3, // 所需要的系统核心支持库的主版本号
    /*m_nRqSysKrnlLibMinorVer*/ 0, // 所需要的系统核心支持库的次版本号

    /*m_szName*/                "数据操作支持库一", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现了数据压缩解压、数据完整性校验、数据加解密支持。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_dp1_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_dp1_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        3, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000数据压缩解压\0""0000数据完整性校验\0""0000数据加解密\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_dp1_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_dp1_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_dp1_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             dp1_ProcessNotifyLib_dp1, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_dp1_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_dp1_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_dp1_global_var;
}

#define DP1_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(DP1_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesdp1[] = { DP1_DEF(DP1_DEF_CMDNAME_STR) };
