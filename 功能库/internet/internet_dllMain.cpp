...
EXTERN_C INT WINAPI internet_ProcessNotifyLib_internet(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define INTERNET_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    INTERNET_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_internet_global_var_fun[] = { INTERNET_DEF(INTERNET_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_internet_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "707ca37322474f6ca841f0e224f4b620", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "互联网支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库用作实现对常用互联网际协议的支持，如SMTP、HTTP、FTP等", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_internet_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_internet_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        3, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000邮件发送\0""0000HTTP及FTP操作\0""0000拨号上网\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_internet_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_internet_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_internet_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             internet_ProcessNotifyLib_internet, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_internet_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_internet_global_var, // 本库预先定义的所有常量
...
EXTERN_C PLIB_INFO WINAPI GetNewInf()
{
    ...
    g_LibInfo_internet_global_var.m_pBeginCmdInfo[21].m_szEgName = "GetUserName";
    return &g_LibInfo_internet_global_var;
}

#define INTERNET_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(INTERNET_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static LPCSTR g_cmdNamesinternet[] = { INTERNET_DEF(INTERNET_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI internet_ProcessNotifyLib_internet(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    case NL_GET_CMD_FUNC_NAMES:
    {
        ...
        g_cmdNamesinternet[21] = ______E_FNENAME(INTERNET_NAME(21, GetUserNameW));
        return (INT)g_cmdNamesinternet;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"internet_ProcessNotifyLib_internet";
    }
