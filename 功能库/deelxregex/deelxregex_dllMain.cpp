...
EXTERN_C INT WINAPI deelxregex_ProcessNotifyLib_deelxregex(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define DEELXREGEX_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    DEELXREGEX_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_deelxregex_global_var_fun[] = { DEELXREGEX_DEF(DEELXREGEX_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_deelxregex_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "6CE139EAF3484af3AE10E402BB263AB8", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    /*m_nMajorVersion*/         2, // 本库的主版本号, 必须大于0
    /*m_nMinorVersion*/         3, // 本库的次版本号
    /*m_nBuildNumber*/          0, // 构建版本号, 无需对此版本号作任何处理

    /*m_nRqSysMajorVer*/        5, // 所需要的易语言系统的主版本号
    /*m_nRqSysMinorVer*/        3, // 所需要的易语言系统的次版本号
    /*m_nRqSysKrnlLibMajorVer*/ 0, // 所需要的系统核心支持库的主版本号
    /*m_nRqSysKrnlLibMinorVer*/ 0, // 所需要的系统核心支持库的次版本号

    /*m_szName*/                "正则表达式支持库(Deelx版)", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "该支持库封装了DEELX正则表达式引擎（v1.2）,实现了支持 单行模式(SINGLELINE)、多行模式(MULTILINE)、全局模式(GLOBAL)、忽略大小写(IGNORECASE)、从右向左(RIGHTTOLEFT)、扩展模式(EXTENDED) 等常见匹配模式。支持与 Perl 兼容的正则表达式语法。注意，本支持库中的所有命令均为高级命令，要求使用者具有一定的正则表达式知识基础。", // 库详细解释
    ...
    /*m_szFax*/                 NULL, // 传真
    /*m_szEmail*/               NULL, // 邮箱
    /*m_szHomePage*/            NULL, // 主页
    /*m_szOther*/               "免费授权给易语言公司发布", // 其他信息

    /*m_nDataTypeCount*/        g_DataType_deelxregex_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_deelxregex_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000全局命令\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_deelxregex_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_deelxregex_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_deelxregex_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             deelxregex_ProcessNotifyLib_deelxregex, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_deelxregex_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_deelxregex_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_deelxregex_global_var;
}

#define DEELXREGEX_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(DEELXREGEX_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesdeelxregex[] = { DEELXREGEX_DEF(DEELXREGEX_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI deelxregex_ProcessNotifyLib_deelxregex(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {
        return (INT)g_cmdNamesdeelxregex;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"deelxregex_ProcessNotifyLib_deelxregex";
    }
