...
EXTERN_C INT WINAPI bmpoperate_ProcessNotifyLib_bmpoperate(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define BMPOPERATE_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    BMPOPERATE_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_bmpoperate_global_var_fun[] = { BMPOPERATE_DEF(BMPOPERATE_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_bmpoperate_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "42305932-06E6-47a5-AC79-8BDCDC58DF61", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "位图操作支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "该支持库实现了对位图（*.bmp）文件数据的操作。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_bmpoperate_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_bmpoperate_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000全局命令\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_bmpoperate_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_bmpoperate_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_bmpoperate_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             bmpoperate_ProcessNotifyLib_bmpoperate, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_bmpoperate_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_bmpoperate_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_bmpoperate_global_var;
}

#define BMPOPERATE_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(BMPOPERATE_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesbmpoperate[] = { BMPOPERATE_DEF(BMPOPERATE_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI bmpoperate_ProcessNotifyLib_bmpoperate(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {
        return (INT)g_cmdNamesbmpoperate;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"bmpoperate_ProcessNotifyLib_bmpoperate";
    }
