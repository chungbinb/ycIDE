...
EXTERN_C INT WINAPI ecalc_ProcessNotifyLib_ecalc(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define ECALC_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ECALC_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_ecalc_global_var_fun[] = { ECALC_DEF(ECALC_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_ecalc_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "306AA9E31B5940399723021A0D782077", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "数值计算支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库提供了常用的数值计算方法以及对大数的运算。注意：本支持库除大数运算外绝大部分的算法均参照于<<C语言数值计算初步>>教材。因为算法的不同，本支持库的计算结果可能和你的预想不吻合，所以在使用前，请您务要考虑这个因素，谢谢！", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_ecalc_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_ecalc_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        0, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           NULL, // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_ecalc_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_ecalc_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_ecalc_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             ecalc_ProcessNotifyLib_ecalc, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_ecalc_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_ecalc_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_ecalc_global_var;
}

#define ECALC_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(ECALC_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesecalc[] = { ECALC_DEF(ECALC_DEF_CMDNAME_STR) };
