...
EXTERN_C INT WINAPI javalib_ProcessNotifyLib_javalib(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define JAVALIB_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    JAVALIB_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_javalib_global_var_fun[] = { JAVALIB_DEF(JAVALIB_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_javalib_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "F3DA9F65E55F47cb8A8DAC95A189F4B1", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "Java支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库使易语言具备了访问Java类库的能力。“Java虚拟机”和“Java本地接口”是本支持库中最重要的两个数据类型。由“Java虚拟机”负责真实Java虚拟机的生命周期管理，而“Java本地接口”则提供Java类库的访问接口。使用时，可直接通过“Java本地接口”的各类方法访问Java类库（即与“Java虚拟机”通信），由支持库本身自动完成“Java虚拟机”的创建和销毁。如果需要特别指定“用户类路径”或“本地库路径”，则必需在使用“Java本地接口”之前显式调用“Java虚拟机.创建()”方法创建Java虚拟机，以传入相关路径参数。\r\n    本支持库要求目标机器中已安装“Java运行时环境”（JRE）。如果在没有配置好Java环境的情况下使用本支持库，所有对象方法都将返回空值（假，0，“”）。\r\n    本支持库安装后，将自动在易语言主菜单中添加一项工具菜单，[工具]-[JavaLib：Java类型签名查询]。这是一个查询“Java 类型签名（Type Signature）”的实用工具。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_javalib_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_javalib_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        0, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           NULL, // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_javalib_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_javalib_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_javalib_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_szzAddInFnInfo*/        "JavaLib: Java类型签名查询\0""查询指定Java类的类型签名(Type Signature)。\0""\0", // 功能详细介绍(仅限一行60字符), 最后由两个空串结束

    /*m_pfnNotify*/             javalib_ProcessNotifyLib_javalib, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_javalib_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_javalib_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_javalib_global_var;
}

#define JAVALIB_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(JAVALIB_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesjavalib[] = { JAVALIB_DEF(JAVALIB_DEF_CMDNAME_STR) };
