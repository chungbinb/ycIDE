...
// 调用格式: SDT_BOOL (Java本地接口).非虚拟对象方法, 命令说明: "根据指定的类和方法标志符调用某对象的具有对象类型返回值的实例（非静态）方法。调用成功返回“真”，失败返回“假”。"
...
// 参数<4>: &返回结果 SDT_INT, 参数说明: "存放方法调用后的返回值——对象标志符。"
// 参数<5>: [参数值 _SDT_ALL], 参数说明: "对应于被调用方法的参数值，注意传入的类型要与该方法实际需要的类型一致。本参数可重复添加。"
// 变长参数, 命令参数表中最后一个参数可以被重复添加
JAVALIB_EXTERN_C void javalib_CallNonvirtualObjectMethod_34_javalib(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf)
{
