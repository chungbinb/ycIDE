...
EXTERN_C INT WINAPI eapi_ProcessNotifyLib_eapi(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define EAPI_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    EAPI_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_eapi_global_var_fun[] = { EAPI_DEF(EAPI_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_eapi_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "F7FC1AE45C5C4758AF03EF19F18A395D", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_nDataTypeCount*/        g_DataType_eapi_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_eapi_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        3, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000系统处理\0""0000网络通信\0""0000窗口操作\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_eapi_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_eapi_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_eapi_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             eapi_ProcessNotifyLib_eapi, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_eapi_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_eapi_global_var, // 本库预先定义的所有常量
...
{
    // 有几个名字变成了宏, 这里需要把命令改成没有A/W的英文命令
    g_LibInfo_eapi_global_var.m_pBeginCmdInfo[16].m_szEgName = "GetFileVersionInfo";
    g_LibInfo_eapi_global_var.m_pBeginCmdInfo[76].m_szEgName = "GetWindowText";
    g_LibInfo_eapi_global_var.m_pBeginCmdInfo[77].m_szEgName = "GetClassName";
    g_LibInfo_eapi_global_var.m_pBeginCmdInfo[86].m_szEgName = "GetDefaultPrinter";
    g_LibInfo_eapi_global_var.m_pBeginCmdInfo[87].m_szEgName = "SetDefaultPrinter";
    return &g_LibInfo_eapi_global_var;
}

#define EAPI_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(EAPI_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static LPCSTR g_cmdNameseapi[] = { EAPI_DEF(EAPI_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI eapi_ProcessNotifyLib_eapi(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {
        // 有几个名字被调用到宏了, 所以需要把这几个名字改一下, 不然静态编译会找不到那几个符号, 这里统一指定使用W版
        g_cmdNameseapi[16] = ______E_FNENAME(EAPI_NAME(16, GetFileVersionInfoW));
        g_cmdNameseapi[76] = ______E_FNENAME(EAPI_NAME(76, GetWindowTextW));
        g_cmdNameseapi[77] = ______E_FNENAME(EAPI_NAME(77, GetClassNameW));
        g_cmdNameseapi[86] = ______E_FNENAME(EAPI_NAME(86, GetDefaultPrinterW));
        g_cmdNameseapi[87] = ______E_FNENAME(EAPI_NAME(87, SetDefaultPrinterW));
        return (INT)g_cmdNameseapi;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"eapi_ProcessNotifyLib_eapi";
    }
