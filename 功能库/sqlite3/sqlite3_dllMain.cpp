...
EXTERN_C INT WINAPI sqlite3_ProcessNotifyLib_sqlite3(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define SQLITE3_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    SQLITE3_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_sqlite3_global_var_fun[] = { SQLITE3_DEF(SQLITE3_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_sqlite3_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "{E5D631FE-E3C9-4eb3-A687-C89598FE6691}", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_nDataTypeCount*/        g_DataType_sqlite3_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_sqlite3_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000数据库\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_sqlite3_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_sqlite3_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_sqlite3_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             sqlite3_ProcessNotifyLib_sqlite3, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_sqlite3_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_sqlite3_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_sqlite3_global_var;
}

#define SQLITE3_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(SQLITE3_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamessqlite3[] = { SQLITE3_DEF(SQLITE3_DEF_CMDNAME_STR) };
