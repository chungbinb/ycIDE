...
#define __XPLIB_NAME(_index, _name) __LIB2_FNE_NAME_LEFT(__E_FNENAME)##_##_name##_##_index##_

// 传递函数名和索引, 拼接成 定义库名_名字_序号_定义库名, 比如 xplib_test_0_xplib
#define XPLIB_NAME(_index, _name) __LIB2_FNE_NAME_LEFT(__XPLIB_NAME(_index, _name))__LIB2_FNE_NAME_LEFT(__E_FNENAME)

// 传递函数名和索引, 拼接成 "定义库名_名字_序号_定义库名", 比如 "xplib_test_0_xplib"
#define XPLIB_NAME_STR(_index, _name) ______E_FNENAME(__XPLIB_NAME(_index, _name))
...
#define XPLIB_DEF(_MAKE) \
    _MAKE(  0, "XP风格", SetXP, "设置当前应用程序的所有窗口组件风格为XP界面风格，成功返回真，失败返回假", 1, _CMD_OS(__OS_WIN), SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1, g_argumentInfo_xplib_global_var + 0)
