...
EXTERN_C INT WINAPI commobj_ProcessNotifyLib_commobj(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define COMMOBJ_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    COMMOBJ_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_commobj_global_var_fun[] = { COMMOBJ_DEF(COMMOBJ_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_commobj_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "{A068799B-7551-46b9-8CA8-EEF8357AFEA4}", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "通用对象支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "目前提供“快速文本对象”“快速字节集对象”等通用对象数据类型，分别实现对“文本”“字节集”等易语言基本数据类型的包装。除了易用性，本库还特别注重大批量数据添加删除时的操作效率。", // 库详细解释
    /*m_dwState*/               _LIB_OS(OS_ALL), // _LIB_OS() | LBS_ 开头常量, 如果是插件,必须包含 LBS_IDE_PLUGIN

    /*m_szAuthor*/              "大有吴涛易语言软件开发有限公司", // 作者
    ...
    /*m_nDataTypeCount*/        g_DataType_commobj_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_commobj_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000命令分类\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_commobj_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_commobj_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_commobj_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             commobj_ProcessNotifyLib_commobj, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_commobj_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_commobj_global_var, // 本库预先定义的所有常量
...
EXTERN_C PLIB_INFO WINAPI GetNewInf()
{
    ...
    g_LibInfo_commobj_global_var.m_pBeginCmdInfo[36].m_szEgName = "ReplaceText";
    return &g_LibInfo_commobj_global_var;
}

#define COMMOBJ_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(COMMOBJ_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static LPCSTR g_cmdNamescommobj[] = { COMMOBJ_DEF(COMMOBJ_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI commobj_ProcessNotifyLib_commobj(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    case NL_GET_CMD_FUNC_NAMES:
    {
        ...
        g_cmdNamescommobj[36] = ______E_FNENAME(COMMOBJ_NAME(36, ReplaceTextW));
        return (INT)g_cmdNamescommobj;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"commobj_ProcessNotifyLib_commobj";
    }
