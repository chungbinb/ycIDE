...
EXTERN_C INT WINAPI btdownload_ProcessNotifyLib_btdownload(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define BTDOWNLOAD_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    BTDOWNLOAD_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_btdownload_global_var_fun[] = { BTDOWNLOAD_DEF(BTDOWNLOAD_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_btdownload_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "32F502732D7A44809082F855618D6BD4", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "BT下载支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库提供BT下载功能。", // 库详细解释
    ...
    /*m_szPhoto*/               "+86(0411)88995831", // 联系电话
    /*m_szFax*/                 "+86(0411)88995834", // 传真
    /*m_szEmail*/               "service@dywt.com.cn", // 邮箱
    /*m_szHomePage*/            "http://dywt.com.cn", // 主页
    /*m_szOther*/               "祝您一帆风顺，心想事成！", // 其他信息

    /*m_nDataTypeCount*/        g_DataType_btdownload_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_btdownload_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000全局命令\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_btdownload_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_btdownload_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_btdownload_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             btdownload_ProcessNotifyLib_btdownload, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_btdownload_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_btdownload_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_btdownload_global_var;
}

#define BTDOWNLOAD_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(BTDOWNLOAD_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesbtdownload[] = { BTDOWNLOAD_DEF(BTDOWNLOAD_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI btdownload_ProcessNotifyLib_btdownload(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {
        return (INT)g_cmdNamesbtdownload;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"btdownload_ProcessNotifyLib_btdownload";
    }
