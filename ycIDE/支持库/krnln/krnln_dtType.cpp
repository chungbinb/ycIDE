...
LIB_DATA_TYPE_INFO g_DataType_krnln_global_var[] = 
{
    ...
    {"驱动器框", "DriverBox", "本属性在设计时不可用，在运行时可读可写。", 0, NULL, LDT_WIN_UNIT, 0, 2, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"目录框", "DirBox", "本属性在设计时不可用，在运行时可读可写。", 0, NULL, LDT_WIN_UNIT, 0, 2, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"文件框", "FileBox", "本属性在设计时不可用，在运行时可读可写。", 0, NULL, LDT_WIN_UNIT, 0, 4, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"颜色选择器", "ColorPicker", "颜色数值为红绿蓝颜色值或“#透明”常量值。", 0, NULL, LDT_WIN_UNIT, 0, 2, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    ...
    {"客户", "Client", "客户是基于连接的网络数据交换的客户方组件，用作向另一方的服务器端组件发送数据。基于连接的数据交换是一种可靠的、允许大数据", 0, NULL, LDT_WIN_UNIT | LDT_IS_FUNCTION_PROVIDER, 0, 4, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    ...
    {"图形按钮", "PicBtn", "可供选择的属性值：", 0, NULL, LDT_WIN_UNIT, 0, 2, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"外部数据库", "ODBCDB", "本组件提供通过ODBC对外部数据库直接操作的方法。", 0, NULL, LDT_WIN_UNIT | LDT_IS_FUNCTION_PROVIDER, 0, 0, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"外部数据提供者", "ODBCProvider", "外部数据提供者用作为数据源提供对外部ODBC数据库的存取操作。它使用外部数据库作为数据的存储仓库，不支持备注和长字节集类", 0, NULL, LDT_WIN_UNIT | LDT_IS_FUNCTION_PROVIDER, 0, 0, (PEVENT_INFO2)NULL, 0, NULL, NullUnit_GetInterface},
    {"对象", "ComObject", "提供对微软公司 COM 对象的支持，注意对象在超出其作用域后会自动被释放，例如：假设对象存在于子程序局部变量中，当子程序", 0, NULL, LDT_IS_FUNCTION_PROVIDER, 0, 0, (PEVENT_INFO2)NULL, 0, NULL, NULL},
    {"变体型", "Variant", "提供对COM接口中Variant数据类型(即VB中的Any数据类型)的支持，它可以容纳文本、数值、逻辑值、日期时间值、C", 0, NULL, LDT_IS_FUNCTION_PROVIDER, 0, 0, (PEVENT_INFO2)NULL, 0, NULL, NULL},
    {"变体类型", "VariantType", "提供变体型中所能够容纳数据类型的枚举值。", 0, NULL, LDT_ENUM, 0, 0, (PEVENT_INFO2)NULL, 0, NULL, NULL},
};
