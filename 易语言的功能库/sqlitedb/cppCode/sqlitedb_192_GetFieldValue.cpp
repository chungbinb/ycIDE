...
// 调用格式: SDT_BOOL (Sqlite记录集).读字段值, 命令说明: "读取当前记录中指定字段的值，并写入第二个参数“字段值”中。执行成功返回“真”，失败返回“假”。"
// 参数<1>: 字段名称或位置 _SDT_ALL, 参数说明: "参数值可以为一个字段名称文本或者一个字段位置数值，字段位置数值从1开始。"
// 参数<2>: &字段值 _SDT_ALL, 参数说明: "其中将存放取得的字段值。如果必要，将自动进行适当的数据类型转换。"
SQLITEDB_EXTERN_C void sqlitedb_GetFieldValue_192_sqlitedb(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf)
{
