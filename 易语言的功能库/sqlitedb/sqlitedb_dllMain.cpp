...
EXTERN_C INT WINAPI sqlitedb_ProcessNotifyLib_sqlitedb(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define SQLITEDB_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    SQLITEDB_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_sqlitedb_global_var_fun[] = { SQLITEDB_DEF(SQLITEDB_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_sqlitedb_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "{295DD8F2-3584-4780-818D-9569AC5BC9D8}", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "Sqlite数据库支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "Sqlite数据库是一个小型关系型文件数据库；跨平台；支持SQL语句、事务、触发器、视图；速度相当快；小巧且不依赖任何额外的驱动程序。Sqlite数据库是本地数据库，不是网络数据库。\r\n    相对于易语言数据库，Sqlite数据库的优势是支持SQL语句、事务、触发器、视图；\r\n    相对于Microsoft Access，Sqlite数据库的优势是跨平台、无需额外驱动；\r\n    相对于其它非本地数据库，如Oracle、DB2、MS SQL Server、MySQL、PostgreSQL，Sqlite数据库具有小巧、速度快的优势。\r\n    内部使用的Sqlite版本是3.2.5。本支持库将只为向后兼容而存在，不会再有版本升级，推荐使用“Sqlite3数据库支持库”。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_sqlitedb_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_sqlitedb_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000数据库\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_sqlitedb_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_sqlitedb_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_sqlitedb_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             sqlitedb_ProcessNotifyLib_sqlitedb, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_sqlitedb_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_sqlitedb_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_sqlitedb_global_var;
}

#define SQLITEDB_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(SQLITEDB_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamessqlitedb[] = { SQLITEDB_DEF(SQLITEDB_DEF_CMDNAME_STR) };
