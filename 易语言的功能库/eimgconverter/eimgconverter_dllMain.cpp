...
EXTERN_C INT WINAPI eimgconverter_ProcessNotifyLib_eimgconverter(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define EIMGCONVERTER_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    EIMGCONVERTER_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_eimgconverter_global_var_fun[] = { EIMGCONVERTER_DEF(EIMGCONVERTER_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_eimgconverter_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "8FA3AA46276847db8F28E57E7FB97B7F", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    /*m_nMajorVersion*/         2, // 本库的主版本号, 必须大于0
    /*m_nMinorVersion*/         0, // 本库的次版本号
    /*m_nBuildNumber*/          4, // 构建版本号, 无需对此版本号作任何处理
    ...
    /*m_szName*/                "图像格式转换支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库用来转换图像格式，目前支持读取的源的格式有：标准JPEG（全部格式）、BMP（目前仅支持24位BMP）、TIFF（单页图像所有格式）、PNG（32位及32位以下）、PCX（8位和24位，RLE压缩）、PCD（取第三幅），目前支持写入的目标格式有：标准JPEG（全部格式）、BMP（24位）、TIFF（灰度和真彩）、PNG（24位）、PPM（文本和二进制模式）、PGM（文本和二进制模式）、PBM（文本和二进制模式）、PCX（24位）、PICS（24位）、SGI（24位）", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_eimgconverter_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_eimgconverter_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000图像转换\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_eimgconverter_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_eimgconverter_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_eimgconverter_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             eimgconverter_ProcessNotifyLib_eimgconverter, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_eimgconverter_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_eimgconverter_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_eimgconverter_global_var;
}

#define EIMGCONVERTER_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(EIMGCONVERTER_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNameseimgconverter[] = { EIMGCONVERTER_DEF(EIMGCONVERTER_DEF_CMDNAME_STR) };
...
EXTERN_C INT WINAPI eimgconverter_ProcessNotifyLib_eimgconverter(INT nMsg, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {
        return (INT)g_cmdNameseimgconverter;
    }
    // 返回处理系统通知的函数名称(PFN_NOTIFY_LIB函数名称), 支持静态编译的动态库必须处理
    case NL_GET_NOTIFY_LIB_FUNC_NAME:
    {
        return (INT)"eimgconverter_ProcessNotifyLib_eimgconverter";
    }
