...
EXTERN_C INT WINAPI downlib_ProcessNotifyLib_downlib(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define DOWNLIB_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    DOWNLIB_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_downlib_global_var_fun[] = { DOWNLIB_DEF(DOWNLIB_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_downlib_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "80CF4A6B3E09425bA57935A3A0E4C473", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "网络传送支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现对多种协议断点续传下载与FTP上传的支持，并提供了丰富的设置方式。当前版本支持HTTP、FTP、MMS多线程下载及断点续传，以及对RTSP协议的单线程下载及断点续传。用户可以根据各自的主机及网络情况进行设置，本支持库可以运行在linux下，详细情况参见例程。注意：当前版本使用了映射文件模式，并且是一次全部映射的方法，所以最大传送不超过一次映射文件的长度。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_downlib_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_downlib_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000全局命令\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_downlib_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_downlib_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_downlib_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             downlib_ProcessNotifyLib_downlib, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_downlib_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_downlib_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_downlib_global_var;
}

#define DOWNLIB_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(DOWNLIB_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesdownlib[] = { DOWNLIB_DEF(DOWNLIB_DEF_CMDNAME_STR) };
