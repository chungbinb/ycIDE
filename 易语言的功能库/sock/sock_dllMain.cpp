...
EXTERN_C INT WINAPI sock_ProcessNotifyLib_sock(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define SOCK_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    SOCK_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_sock_global_var_fun[] = { SOCK_DEF(SOCK_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_sock_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "A6B983789F624b2cBDFD7D671249C097", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    /*m_nMajorVersion*/         2, // 本库的主版本号, 必须大于0
    /*m_nMinorVersion*/         0, // 本库的次版本号
    /*m_nBuildNumber*/          2, // 构建版本号, 无需对此版本号作任何处理
    ...
    /*m_szName*/                "网络通讯支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现对网络通讯的支持", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_sock_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_sock_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000网络通讯\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_sock_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_sock_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_sock_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             sock_ProcessNotifyLib_sock, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_sock_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_sock_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_sock_global_var;
}

#define SOCK_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(SOCK_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamessock[] = { SOCK_DEF(SOCK_DEF_CMDNAME_STR) };
