...
// 调用格式: SDT_INT (Java本地接口).取成员标志符, 命令说明: "返回类的实例成员（非静态成员）的成员标志符，该成员由其名称及签名指定。失败返回0。本方法可自动将未初始化的类初始化。本方法不能用于获取数组的长度，应调用“取数组长度()”方法。调用“取成员()”“置成员()”系列方法时都将用到本方法返回的成员标志符。"
...
JAVALIB_EXTERN_C void javalib_GetFieldID_19_javalib(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf)
{
