...
EXTERN_C INT WINAPI portio_ProcessNotifyLib_portio(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define PORTIO_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    PORTIO_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_portio_global_var_fun[] = { PORTIO_DEF(PORTIO_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_portio_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "2F48E8AD71534EB79B5AD580D189231A", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "端口访问支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现了对计算机端口的直接访问支持，注意在执行本支持库内命令前计算机上必须要安装有第三方免费软件 port95nt.exe，下载地址 http://www.dywt.com.cn/port95nt.exe ", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_portio_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_portio_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        2, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000端口访问\0""0000并口设置\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_portio_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_portio_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_portio_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             portio_ProcessNotifyLib_portio, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_portio_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_portio_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_portio_global_var;
}

#define PORTIO_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(PORTIO_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesportio[] = { PORTIO_DEF(PORTIO_DEF_CMDNAME_STR) };
