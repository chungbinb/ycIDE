...
EXTERN_C INT WINAPI esslayer_ProcessNotifyLib_esslayer(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define ESSLAYER_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ESSLAYER_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_esslayer_global_var_fun[] = { ESSLAYER_DEF(ESSLAYER_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_esslayer_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "931A3D5BC194493f97A2830FE4D7A32D", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_szName*/                "保密通讯支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现在网络上保密通讯的目的。由于一些机密数据在网上传输过程中可能会被一些别有用心的人截获破坏，以致对数据的拥有者造成经济上或者是物质上的损失，这样就要求数据在网络上传输的过程中不会被他人利用破坏。本支持库的设计和实现就是为了完成在网络上保密通讯的目的，注意本支持库仅仅是完成对数据在传输的过程中的保护。理论上，在用户的私钥不公开的情况下，该通讯方式是绝对安全的，即使算法的设计者也不能做出任何破坏。本支持库使用也非常简单，在客户端和服务程序连接前，设置好相应密钥，在实际的使用过程中，用户不必关心加密解密的过程，正常发送和接收数据就可以了。为了完全理解该支持库，建议使用者查阅有关“安全套接字层(SSL)”的原理，本支持库就是对该原理的实现", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_esslayer_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_esslayer_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        0, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           NULL, // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_esslayer_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_esslayer_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_esslayer_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             esslayer_ProcessNotifyLib_esslayer, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_esslayer_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_esslayer_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_esslayer_global_var;
}

#define ESSLAYER_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(ESSLAYER_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesesslayer[] = { ESSLAYER_DEF(ESSLAYER_DEF_CMDNAME_STR) };
