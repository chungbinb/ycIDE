...
EXTERN_C INT WINAPI mysql_ProcessNotifyLib_mysql(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define MYSQL_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    MYSQL_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_mysql_global_var_fun[] = { MYSQL_DEF(MYSQL_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_mysql_global_var = 
{
    ...
    /*m_szName*/                "MySQL支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             "本支持库实现对MySQL数据库的支持，在使用本支持库前请学习MySQL相关知识，如果出错可以通过“取错误文本()”查看出错信息。从易语言5.0开始本库有内部调整，请对照命令说明检查“断开Mysql”“释放记录集”相关代码，确认代码书法无误。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_mysql_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_mysql_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        1, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000MySql操作\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_mysql_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_mysql_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_mysql_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             mysql_ProcessNotifyLib_mysql, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_mysql_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_mysql_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_mysql_global_var;
}

#define MYSQL_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(MYSQL_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesmysql[] = { MYSQL_DEF(MYSQL_DEF_CMDNAME_STR) };
