...
EXTERN_C INT WINAPI ogrelib_ProcessNotifyLib_ogrelib(INT nMsg, DWORD dwParam1, DWORD dwParam2);
...
#define OGRELIB_DEF_CMD_PTR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    OGRELIB_NAME(_index, _szEgName),

PFN_EXECUTE_CMD g_cmdInfo_ogrelib_global_var_fun[] = { OGRELIB_DEF(OGRELIB_DEF_CMD_PTR) };

static LIB_INFO g_LibInfo_ogrelib_global_var = 
{
    /*m_dwLibFormatVer*/        LIB_FORMAT_VER, // 库格式号, 应该等于LIB_FORMAT_VER
    /*m_szGuid*/                "2EAE87405D754ad780D8FE57432002EA", // 对应于本库的唯一GUID串, 不能为NULL或空, 库的所有版本此串都应相同
    ...
    /*m_nRqSysMinorVer*/        9, // 所需要的易语言系统的次版本号
    /*m_nRqSysKrnlLibMajorVer*/ 3, // 所需要的系统核心支持库的主版本号
    /*m_nRqSysKrnlLibMinorVer*/ 9, // 所需要的系统核心支持库的次版本号

    /*m_szName*/                "DirectX3D支持库", // 库名, 不能为NULL或空
    /*m_nLanguage*/             __GBK_LANG_VER, // 库所支持的语言
    /*m_szExplain*/             " 本支持库实现了对DirectX9 3D的支持。", // 库详细解释
    ...
    /*m_nDataTypeCount*/        g_DataType_ogrelib_global_var_count, // 本库中自定义数据类型的数目
    /*g_DataType_web*/          g_DataType_ogrelib_global_var, // 本库中所有的自定义数据类型

    /*m_nCategoryCount*/        3, // 全局命令类别数目, 可为0
    /*m_szzCategory*/           "0000纹理\0""0000模型\0""0000其它\0""\0", // 全局命令类别数目, 可为0

    /*m_nCmdCount*/             g_cmdInfo_ogrelib_global_var_count, // 本库中提供的所有命令(全局命令及对象命令)的数目, 可为0
    /*m_pBeginCmdInfo*/         g_cmdInfo_ogrelib_global_var, // 本库中提供的所有命令
    /*m_pCmdsFunc*/             g_cmdInfo_ogrelib_global_var_fun, // 指向每个命令的实现代码首地址, 可为NULL
    ...
    /*m_pfnNotify*/             ogrelib_ProcessNotifyLib_ogrelib, // 不能为NULL,和易语言通讯的子程序
    ...
    /*m_nLibConstCount*/        g_ConstInfo_ogrelib_global_var_count, // 本库预先定义的所有常量数量
    /*m_pLibConst*/             g_ConstInfo_ogrelib_global_var, // 本库预先定义的所有常量
...
{
    return &g_LibInfo_ogrelib_global_var;
}

#define OGRELIB_DEF_CMDNAME_STR(_index, _szName, _szEgName, _szExplain, _shtCategory, _wState, _dtRetValType, _wReserved, _shtUserLevel, _shtBitmapIndex, _shtBitmapCount, _nArgCount, _pBeginArgInfo) \
    ______E_FNENAME(OGRELIB_NAME(_index, _szEgName)),

// 函数名, 这个给静态编译使用
static const LPCSTR g_cmdNamesogrelib[] = { OGRELIB_DEF(OGRELIB_DEF_CMDNAME_STR) };
